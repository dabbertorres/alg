...
public class Parser {
    ...
    private static Expression addOperator(Token op, Expression lhs, Expression rhs) {
        return switch (op) {
            case PLUS -> new Expression.Addition(lhs, rhs);
            case MINUS -> new Expression.Subtraction(lhs, rhs);
            case MULTIPLY -> new Expression.Multiplication(lhs, rhs);
            case DIVIDE -> new Expression.Division(lhs, rhs);
            case POWER -> new Expression.Power(lhs, rhs);
            default -> throw new UnexpectedTokenException(op.toString());
        };
    }

    private static Expression addFunction(Token func, Expression subExpr) {
        return switch (func) {
            case SINE -> new Expression.Sine(subExpr);
            case COSINE -> new Expression.Cosine(subExpr);
            case TANGENT -> new Expression.Tangent(subExpr);
            case LOG -> new Expression.Log(subExpr);
            case NATURAL_LOG -> new Expression.NaturalLog(subExpr);
            case SQUARE_ROOT -> new Expression.SquareRoot(subExpr);
            default -> throw new UnexpectedTokenException(func.toString());
        };
    }
